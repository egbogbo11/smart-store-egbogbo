...
def remove_outliers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Ensure DiscountPercent is numeric
    df['DiscountPercent'] = pd.to_numeric(df['DiscountPercent'], errors='coerce')

    # Remove rows where DiscountPercent is NaN
    df = df.dropna(subset=['DiscountPercent'])

    # Apply the range filter for SaleAmount
    df = df[(df['SaleAmount'] >= 0) & (df['SaleAmount'] <= 10_000)]

    # Remove rows where DiscountPercent is greater than 20
    df = df[df['DiscountPercent'] <= 20]
...
def validate_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Ensure that DiscountPercent is numeric
    df['DiscountPercent'] = pd.to_numeric(df['DiscountPercent'], errors='coerce')
    
    # Handle rows where BonusPoints are negative
    invalid_bonus_points = df[df['DiscountPercent'] < 0].shape[0]
    logger.info(f"Found {invalid_bonus_points} sales with negative bonus points") 

    # Change negative DiscountPercent to positive and convert to integer
    df['DiscountPercent'] = df['DiscountPercent'].abs()
